...
while 1:
	while stop:
		stop = db.selectLast('flags','stop')
		#print 'stop = ', str(stop)
		time.sleep(2)
	else:
		start = db.selectLast('flags','start')
	...
	if start == 1:
		...
		sym = open('/var/www/demo/ASM1/symulacje.txt', 'a')
                ...
                sym.close()
		#print 'symulacja zakonczona ' + str(datetime.datetime.now())

	else:
		stop = db.selectLast('flags','stop')
        	print 'Przerwanie obliczen. stop = ', str(stop)
