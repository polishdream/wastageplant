...
class Params(object):
	def __init__(self,qin):
		...
		self.updtInv = int(900.0/db.selectLast('params','tp'))	#interwal aktualizacji parametrow z pliku (15 min wg benchmarka)
		self.nextUpdt = self.updtInv	#indeks nastepnej aktualizacji

		#BIOREACTOR, SETTLER
		self.q_in= qin * self.tp #m3/iteracje
		self.q_ir = db.selectLast('params','qir') * self.q_in #sprzezenie zwrotne wewnetrzne
		self.q_e = 0.979 * self.q_in
		self.q_w = 0.021 * self.q_in
		self.q_r = self.q_in
		...
		self.nr_layers = 10	#ilosc warstw osadnika
		self.A = 1500.0 #powierzchnia m2
		self.V = 6000.0 #objetosc m3
		self.h_f = 0.4
		self.X = [0.0] * 10 
		#self.X = [12.5,18.1,29.5,69.0,356.0,356.0,356.0,356.0,356.0,6394.0]
		self.dX = [0.0] * 10
		self.X_f = 0.0
		self.X_e = 12.5
		self.X_u = 6394.0
		...
		self.xTab = []
		self.yTab = []
	        self.fig1 = plt.figure() #figure
		self.step = 300 #co ile wartosci pobiera dana do wykresu i aktualizuje go
		self.kompTab = ['C1','C2','C3','C4','C5']
	...
	def updateParams(self,var,cUp):
                self.q_in= var[cUp][14] * self.tp #m3/iteracje
                self.q_ir = db.selectLast('params','qir') * self.q_in #sprzezenie zwrotne wewnetrzne
                self.q_e = 0.979 * self.q_in
                self.q_w = 0.021 * self.q_in
                self.q_r = self.q_in
...
class Kompartment(Params):	#C
	...
	def przepiszNew(self,var,count):
		self.s_o = var[count][8]
                self.s_no = var[count][9]
                self.s_nd = var[count][11]
                self.s_nh = var[count][10]
                self.s_s = var[count][2]
                self.s_i = var[count][1]
                self.x_h = var[count][5]
                self.x_a = var[count][6]
                self.x_s = var[count][4]
                self.x_nd = var[count][12]
                self.x_p = var[count][7]
                self.x_i = var[count][3]
...
path = '/Users/Mateusz/Desktop/beaglebone/projekt/baza.db' #sciezka do bazy danych
	
db.connect(path) #polaczenie z baza

variables = np.loadtxt('Inf_strm_2006.txt')
params = Params(variables[0][14])
...
C3 = Kompartment(params,1.72,6.54,0.829,5.55,1.15,30.0,2557.0,149.0,64.9,4.39,45.0,1149.0,vol=1333.0,kla=params.kla) #ostatnie parametry K_la, vol opcjonalne - domyslnie 0
C4 = Kompartment(params,2.43,9.30,0.767,2.97,0.995,30.0,2559.0,150.0,55.7,3.88,451.0,1149.0,vol=1333.0,kla=params.kla)
C5 = Kompartment(params,0.491,10.4,0.688,1.73,0.889,30.0,2559.0,150.0,49.3,3.53,452.0,1149.0,vol=1333.0,kla=140)
...
C_in = Kompartment(params,variables[0][8],variables[0][9],variables[0][11],variables[0][10],variables[0][2],variables[0][1],variables[0][5],variables[0][6],variables[0][4],variables[0][12],variables[0][7],variables[0][3])
C_ir = Kompartment(params,0.491,10.4,0.688,1.73,0.889,30.0,2559.0,150.0,49.3,3.53,452.0,1149.0) #C_ir = C5
#C_ir = Kompartment(params,0.0,0.0,6.95,31.56,69.5,30.00,28.17,0.0,202.32,10.59,0.0,51.20)
#C_r = Kompartment(params,0.0,0.0,6.95,31.56,69.5,30.00,params.cf*C_in.x_h,params.cf*C_in.x_a,params.cf*C_in.x_s,params.cf*C_in.x_nd,params.cf*C_in.x_p,params.cf*C_in.x_i)

#settler 
C_u = Kompartment(params,0.491,10.4,0.688,1.73,0.889,30.0,5005.0,293.0,96.4,6.90,884.0,2247.0)
C_r = Kompartment(params,0.491,10.4,0.688,1.73,0.889,30.0,5005.0,293.0,96.4,6.90,884.0,2247.0) #C_r = C-u
...
while 1:
	while params.stop:
		params.stop = db.selectLast('flags','stop')
		print 'stop = ', str(params.stop)
		time.sleep(2)
	else:
		params.start = db.selectLast('flags','start')
		print 'Rozpoczynam dzialanie. start = ', str(params.start)
	...
	if params.start == 1:
		...
		#zapis do bazy
			...
			params.updateFlag = db.selectLast('flags','refresh')
			if params.updateFlag == 1:
				params.updateParams()
				params.updateFlag = 0
				db.cur.execute('UPDATE flags SET start = ' + str(params.start) + ',stop = ' + str(params.stop) + ',refresh=0 WHERE id = 1')
				db.commitChanges()
				print 'Zauktualizowano'

			if licz == (params.nextUpdt-1):
				params.updateParams(variables,params.nextUpdt/params.updtInv)
				C_in.przepiszNew(variables,params.nextUpdt/params.updtInv)
				params.nextUpdt = params.nextUpdt + params.updtInv
				#print 'aktualizacja z pliku'
		...
		params.stop = db.selectLast('flags','stop')
        	print 'Przerwanie obliczen. stop = ', str(params.stop)

#zamkniecie polaczenia
db.closeConn()
print 'zakonczono bio.py ' + str(datetime.datetime.now())
